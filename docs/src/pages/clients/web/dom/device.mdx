...
> A **device** is a [DOM](/clients/web/dom) [node](/clients/web/dom/node)
that hosts [services](/clients/web/dom/service). A device is hosted in a [bus](/clients/web/dom/bus).

<DeviceList showFirmware={true} showTemperature={true} />
...
Retreives the service by **service number** in the device. Undefined if not found.

```javascript
const service = device.service(pkt.service_number)
```
...
[Service](/clients/web/dom/service), [Register](/clients/web/dom/register), [Bus](/clients/web/dom/bus)