...
> A **bus** is the top-level [DOM](/clients/web/dom) [node](/clients/web/dom/node).
It processes the incoming and outcoming packets, 
handles the connection to the underlying communiction transport
and maintains a set of active [devices](/clients/web/dom/device) nodes.
...
You can query the bus at any to receive a snapshot of the [devices](./device) DOM.
