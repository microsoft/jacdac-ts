...
class BaseRegisterUpdateEvent extends BaseRegisterEvent {
    constructor(
        ...
        protected options: RegisterUpdateOptions
    ) {
        super(busRegister)
    }
    ...
    protected processTrigger(register: JDRegister) {
        ...
        const precondition =
            this.options.preRequiredRange === undefined ||
            (thisValue !== undefined &&
                thisValue >= this.options.preRequiredRange[0] &&
                thisValue <= this.options.preRequiredRange[1])

        // whether or not toRange is defined, the current sample must be valid
        const triggered =
            thisValue !== undefined &&
            (this.options.triggerRange === undefined ||
                (thisValue >= this.options.triggerRange[0] &&
                    thisValue <= this.options.triggerRange[1]))
        ...
        if (triggered) {
            ...
            throw new RegisterPreConditionError(
                `register value ${this.register.name} = ${thisValue} not in precondition ${this.options.preRequiredRange}`
            )
        } else {
            return false
        }
...
class RegisterUpdateEventHold extends BaseRegisterUpdateEvent {
    constructor(
        ...
        assert(
            ...
            "may not have undefined trigger range with .hold()"
        )
    }
    ...
    protected processHold(register: JDRegister) {
        ...
        if (
            ...
            throw new RegisterPreConditionError(
                `register value ${this.register.name} = ${thisValue} not in hold condition ${this.options.triggerRange}`
            )
        }
    }
}
