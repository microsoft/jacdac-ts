...
class SpiTransport extends Transport
{
    ...
    constructor(readonly controller: Rpio, readonly options: SpiTransportOptions)
    {
        super("spi", options)
    ...
    private async internalTransportConnectAsync(background?: boolean): Promise<void> {
        ...
        const { HIGH, LOW, POLL_HIGH, PULL_DOWN, INPUT, OUTPUT } = this.controller;
        ...
        this.controller.spiSetClockDivider(128);           /* AT93C46 max is 2MHz, 128 == 1.95MHz */
        ...
        if (rxReady)
        {
            ...
            while (framep < XFER_SIZE)
            {
                ...
                */
                ...
                {
                    console.log(`packet overflow ${framep} + ${sz} > ${XFER_SIZE}`);
                    break;
                }
    ...
    private async attemptTransferBuffers(txqueue: Uint8Array, rxqueue: Uint8Array)
    {
        // attempt transfer
 ...
 * @returns 
 */
export function createNodeSPITransport(controller: Rpio, options?: SpiTransportOptions): Transport {
    if (!options) {
        options = {
