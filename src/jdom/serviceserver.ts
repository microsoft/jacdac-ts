...
export default class JDServiceServer extends JDEventSource {
    ...
    register<TValues extends any[] = any[]>(identifier: number): JDRegisterServer<TValues> {
        return this._registers.find(reg => reg.identifier === identifier) as JDRegisterServer<TValues>;
    }

    protected addRegister<TValues extends any[] = any[]>(identifier: number, defaultValue?: TValues): JDRegisterServer<TValues> {
        let reg = this._registers.find(r => r.identifier === identifier) as JDRegisterServer<TValues>;
        if (!reg) {
