...
import { ArcadeGamepadButton, JD_DEVICE_DISCONNECTED_DELAY } from "../jdom/constants";
import DeviceHost from "../jdom/devicehost";
import ArcadeGamepadServiceHost, { defaultButtons } from "./arcadegamepadservicehost";
...
export default class GamepadHostManager extends JDClient {
    private hosts: {
        ...
        service: ArcadeGamepadServiceHost;
        timestamp: number;
    }[] = [];
    ...
    private handleGamepadDisconnected(event: GamepadEvent) {
        ...
        if (host) {
            this.bus.removeDeviceHost(host.deviceHost);
    ...
    private update() {
        ...
        for (let i = 0; i < gamepads?.length; ++i) {
            ...
            if (!host) {
                const service = new ArcadeGamepadServiceHost(standardButtons);
                const deviceHost = new DeviceHost([service]);
                this.bus.addDeviceHost(deviceHost);
